...
import { HeartIcon, MessageCircle, SendIcon } from "lucide-react"; // ← import SendIcon
...
export default function PostCard({ post, user }) {
 ...
 const handleLike = async () => {
  ...
  try {
    await toggleLike(post._id); // Send API request
  } catch (err) {
  ...
  // console.log("Post commentes ",post?.comments[post?.comments?.length - 1]?.userId?.name);
  ...
  return (
    <Card className="overflow-hidden space-y-4">
      <CardContent className="p-4 sm:p-6">
        ...
        <p className="mt-2 text-md text-foreground break-words mb-2">
          {post?.content}
        </p>
        ...
        {showComments && post.comments?.length > 0 && (
          <div className="space-y-4 pt-4 border-t h-[15rem] overflow-y-scroll no-scrollbar">
            {(post?.comments).map((c,index) => (
              <div key={c?._id ?? index} className="flex space-x-3 py-4">
